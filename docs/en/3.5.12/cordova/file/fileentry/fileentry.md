...
Represents a file on a file system, as defined in the [W3C Directories and Systems](http://www.w3.org/TR/file-system-api/) specification. Further examples can be found in the [html5Rocks fileSystem tutorial](http://www.html5rocks.com/en/tutorials/file/filesystem/).
...
- __successCallback (required)__: A callback that is passed a `Metadata` object. _(Function)_
...
- __parent (required)__: The parent directory to which to move the file. _(DirectoryEntry)_
...
- __parent (required)__: The parent directory to which to copy the file. _(DirectoryEntry)_
...
Attempting to access the the file directly via `fileEntry.toURL()` in a WebView that is served via `localhost` will result in a CORS issue on iOS. Instead, to access the file you have to first move it into the read-writable portion of the application files.
...
- __successCallback (required)__: A callback that executes after the file has been deleted.  Invoked with no parameters. _(Function)_
...
- __successCallback (required)__: A callback that is passed the file's parent `DirectoryEntry`. _(Function)_
...
- __successCallback (required)__: A callback that is passed a `FileWriter` object. _(Function)_
...
- __successCallback (required)__: A callback that is passed a `File` object. _(Function)_
