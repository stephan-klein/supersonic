...
- __defaultText__: Default textbox input value (`String`) (Optional, however, due to a known Cordova issue, the default text is set to "Default text", if left empty. To display the prompt without the default text, set the `defaultText` to `new String()`.)
